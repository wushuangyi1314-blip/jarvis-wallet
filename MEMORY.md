...
| Agent | 职责范围 |
|-------|---------|
| **运营 Agent** | 写文章、研究工具、填写 frontmatter |
| **RD Agent** | 所有技术开发任务：CSS 修改、Hugo 模板、部署配置、bug 修复等 |
| **PM Agent** | 产品规划、需求文档、流程设计 |
| **QA Agent** | 测试验证、样式检查 |

**⚠️ 重要原则：**

1. **技术开发类任务必须分配给 RD Agent**，禁止自己动手做。
2. **纯技术类问题由 RD Agent 自己评估决定**，不需要用户（贾维斯）评估。

   - 含义：技术方案选型、bug 原因分析、框架选择等技术问题，RD Agent 有能力独立判断，不需要我先评估再执行。
   - 我只负责把任务分配清楚，RD Agent 自己搞定。

3. 需要我介入的情况：
   - 运营/产品相关的工作（内容、流程、设计方案）
   - 跨 Agent 的协调
   - 业务或战略层面的决策
