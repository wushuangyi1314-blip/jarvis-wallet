...
| 2026-04-24 19:24 | 小说工厂目录重组 | ✅ 完成 | 通用规范文档重组+删除废弃目录+novels/删除 |
| 2026-04-24 19:27 | 文档体系v5更新 | ✅ 完成 | 36个文件/11个模板/已同步 |
...
## 小说工厂文档修订规则（2026-04-21 新增）

### 背景

小说工厂涉及30个文档（14个通用规范文档+10个项目文档模板+工作流+文档体系+状态文件机制）。为确保文档迭代有序、可追溯、可回退，制定以下规则。
...
| **规则2：版本迭代** | 每次修改，文档名字需要加上版本迭代后缀（如v1→v2） |
| **规则3：原文件保留** | 每次修改，保留原文件，复制一个新文件再改。新版本不符合预期时，可删掉新版本，继续在老版本基础上调整 |
| **规则4：版本历史** | 每个文档必须有版本历史记录，记载每次变更说明 |
| **规则5：Git同步** | 每次文档修改后，需要git commit + push，同步到GitHub |
