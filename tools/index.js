...
class TemporalReasoner {
  ...
  getEventsBefore(time) {
    ...
    return this.timeline.filter((e) => new Date(e.timestamp).getTime() < targetTime);
  }
  ...
  getEventSequence(conceptId) {
    return this.timeline.filter(
      (e) => e.conceptId === conceptId || e.relatedConcepts?.includes(conceptId)
    );
  }
}
  ...
  consolidate() {
    ...
    for (const [id, concept] of this.nodes) {
      ...
      const importanceScore =
        concept.importance * 0.4 + (concept.accessCount / 100) * 0.3 + (1 / (age + 1)) * 0.3;
