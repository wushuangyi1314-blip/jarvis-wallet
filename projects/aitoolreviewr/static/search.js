  ...
  if (searchInput) {
    searchInput.addEventListener('focus', function() {
      if (searchInput.value.trim()) {
        loadSearchData().then(function() { displayResults(performSearch(searchInput.value.trim()), searchResults); });
      }
    });
    ...
    searchInput.addEventListener('input', function(e) {
      ...
      if (q.length >= 2) {
        loadSearchData().then(function() { displayResults(performSearch(q), searchResults); });
      } else {
  ...
  if (heroInput) {
    heroInput.addEventListener('keydown', function(e) {
      if (e.key === 'Enter') {
        ...
        if (q && searchInput) { searchInput.value = q; searchInput.focus(); }
      }
    });
  }
