...
def _resolve_wechat_creds() -> tuple[str, str]:
    """从 config.json 或环境变量获取微信凭证"""
    wechat = CONFIG.get("wechat", {})
    app_id = wechat.get("app_id") or ""
    app_secret = wechat.get("app_secret") or ""
    if not app_id:
        for key in ("WECHAT_APP_ID", "WECHAT_APPID"):
            app_id = os.environ.get(key, "")
            if app_id:
                break
    if not app_secret:
        for key in ("WECHAT_APP_SECRET", "WECHAT_APPSECRET"):
            app_secret = os.environ.get(key, "")
            if app_secret:
                break
...
def get_access_token():
    ...
    if not app_id or not app_secret:
        print("错误: 未配置微信凭证。请在 config.json 或 ~/.openclaw/.env 中设置 WECHAT_APP_ID / WECHAT_APP_SECRET")
        sys.exit(1)
...
def main():
    ...
    if args.input:
        ...
        theme = args.theme
        if not theme:
            gallery_theme_file = Path(CONFIG["output_dir"]).expanduser() / "selected-theme.txt"
        ...
        # 从 format.py 输出中找到目录
        output_base = Path(CONFIG["output_dir"]).expanduser()
